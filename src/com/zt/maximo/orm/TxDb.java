...
public class TxDb {
	...
	private static HashMap<String, TxDb> daoMap = new HashMap<String, TxDb>();
	
	private SQLiteDatabase db;
	private DaoConfig config;
	
	private TxDb(DaoConfig config){
		if(config == null)
	...
	private synchronized static TxDb getInstance(DaoConfig daoConfig) {
		TxDb dao = daoMap.get(daoConfig.getDbName());
		if(dao == null){
			dao = new TxDb(daoConfig);
	 ...
	 */
	public static TxDb create(Context context,boolean isDebug){
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 * @param dbName 数据库名称
	 */
	public static TxDb create(Context context,String dbName){
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 */
	public static TxDb create(Context context,String dbName,boolean isDebug){
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
		config.setDebug(isDebug);
	 ...
	 * @return
	 */
	public static TxDb create(Context context,String dbName,boolean isDebug,int dbVersion,DbUpdateListener dbUpdateListener){
		/**
